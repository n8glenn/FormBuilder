...
open class FBDateView: UIView
{
    ...
    open func updateDisplay(date:Date, buttonColor:UIColor, dateType:FBDateType)
    {
        ...
        self.button?.addTarget(self, action: #selector(okButtonPressed), for: UIControl.Event.touchUpInside)
        _buttonColor = buttonColor
        self.button?.backgroundColor = _buttonColor
        self.button?.setTitle("Ok", for: UIControl.State.normal)
        self.button?.setTitleColor(UIColor.white, for: UIControl.State.normal)
        self.addSubview(self.button!)
        // create delete button
        self.deleteButton = UIButton()
        self.deleteButton!.layer.cornerRadius = 7.5
        self.deleteButton?.addTarget(self, action: #selector(deleteButtonPressed), for: UIControl.Event.touchUpInside)
        _buttonColor = buttonColor
        self.deleteButton?.backgroundColor = _buttonColor
        self.deleteButton?.setImage(UIImage.init(named: "trash-white", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
        self.deleteButton?.setTitleColor(UIColor.white, for: UIControl.State.normal)
        ...
        self.datePicker?.addTarget(self, action: #selector(dateChanged), for: UIControl.Event.valueChanged)
