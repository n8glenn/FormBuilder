...
open class FBSectionHeaderView: UIView
{
    ...
    open func updateDisplay(index:Int, section:FBSection)
    {
        ...
        if (self.collapsible)
        {
            ...
            if (self.collapsed)
            {
                self.collapseButton?.setImage(UIImage.init(named: "right", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            }
            else
            {
                self.collapseButton?.setImage(UIImage.init(named: "down", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            }
            self.collapseButton?.addTarget(self, action: #selector(collapsePressed), for: UIControl.Event.touchUpInside)
        ...
        if (self.allowsAdd)
        {
            ...
            self.addButton?.setImage(UIImage.init(named: "plus", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            self.addButton?.addTarget(self, action: #selector(addPressed), for: UIControl.Event.touchUpInside)
        ...
        if (self.allowsRemove)
        {
            ...
            self.removeButton?.setImage(UIImage.init(named: "minus", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            self.removeButton?.addTarget(self, action: #selector(removePressed), for: UIControl.Event.touchUpInside)
        ...
        if (self.section?.editable == true)
        {
            ...
            {
                self.editButton?.setImage(UIImage.init(named: "edit", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            }
            else
            {
                self.editButton?.setImage(UIImage.init(named: "save", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            }
            self.editButton?.addTarget(self, action: #selector(editPressed), for: UIControl.Event.touchUpInside)
            self.backgroundView?.addSubview(self.editButton!)
            self.cancelButton = UIButton()
            self.cancelButton?.setImage(UIImage.init(named: "cancel", in: bundle, compatibleWith: nil), for: UIControl.State.normal)
            self.cancelButton?.addTarget(self, action: #selector(cancelPressed), for: UIControl.Event.touchUpInside)
