    ...
    FBSectionHeaderDelegate
{
    ...
    func editSelected(section:Int)
    {
        ...
        self.tableView?.reloadSections([section], with: UITableView.RowAnimation.fade)
    }
    ...
    open func validationFailed(exceptions:Array<FBException>)
    {
        ...
        let alert:UIAlertController = UIAlertController(title: "Validation Failed", message: message, preferredStyle: UIAlertController.Style.alert)
    ...
    func addItem(section: Int, type: FBFieldType)
    {
        ...
        case FBFieldType.Image:
            let alert:UIAlertController = UIAlertController(title: "Image Label Text", message: "Enter text for the label of this image.", preferredStyle: UIAlertController.Style.alert)
            alert.addTextField(
            ...
            let action = UIAlertAction(title: "Ok",
                                       ...
                                       handler: {[weak self]
                                        ...
                                        if let textFields = alert.textFields{
                                            ...
                                            self?.tableView!.insertRows(at: [IndexPath(row: currentSection.visibleLines().count - 1, section:section)], with: UITableView.RowAnimation.none)
        ...
        case FBFieldType.Label:
            let alert:UIAlertController = UIAlertController(title: "Label Text", message: "Enter text for the label.", preferredStyle: UIAlertController.Style.alert)
            alert.addTextField(
                configurationHandler: {(textField: UITextField!) in
        ...
        case FBFieldType.Signature:
            let alert:UIAlertController = UIAlertController(title: "Signature Label Text", message: "Enter text for the label of this signature.", preferredStyle: UIAlertController.Style.alert)
            alert.addTextField(
                configurationHandler: {(textField: UITextField!) in
    ...
    func removeItem(indexPath: IndexPath, type: FBFieldType)
    {
        ...
        {
        case FBFieldType.Section:
            let alert:UIAlertController = UIAlertController(title: "Remove", message: "Remove this section?", preferredStyle: UIAlertController.Style.alert)
    ...
    func lineHeightChanged(line: FBLine)
    {
        ...
        for section in self.form!.visibleSections()
        {
            for l in section.visibleLines()
            {
                ...
                {
                    self.tableView!.reloadRows(at: [IndexPath.init(row: j, section: i)], with: UITableView.RowAnimation.none)
                    return
                }
                j += 1
            }
